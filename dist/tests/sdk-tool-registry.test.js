...
import { createToolRegistry, toolRegistryAsResolver, UnknownToolError, } from "../src/sdk/index.js";
const ok = async () => "ok";
...
describe("toolRegistryAsResolver — shim to ToolResolver (G3a)", () => {
    ...
    it("throws UnknownToolError for missing tools", async () => {
        const r = createToolRegistry();
        const resolver = toolRegistryAsResolver(r);
        await assert.rejects(resolver("missing", {}, new AbortController().signal), (err) => {
            assert.ok(err instanceof UnknownToolError);
            assert.equal(err.toolName, "missing");
            return true;
        });
    });
});
//# sourceMappingURL=sdk-tool-registry.test.js.map