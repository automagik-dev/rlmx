...
describe("buildGeminiOnPayload", () => {
    it("returns undefined for non-Google provider", () => {
        const hook = buildGeminiOnPayload({ ...BASE_CONFIG, googleSearch: true }, "anthropic");
    ...
    it("injects googleSearch tool", () => {
        const hook = buildGeminiOnPayload({ ...BASE_CONFIG, googleSearch: true }, "google");
        assert.ok(hook);
        const payload = { config: {} };
        const result = hook(payload);
        assert.ok(result.config.tools);
        assert.ok(result.config.tools.some((t) => t.googleSearch));
    });
    it("injects urlContext tool", () => {
        const hook = buildGeminiOnPayload({ ...BASE_CONFIG, urlContext: true }, "google");
        assert.ok(hook);
        const payload = { config: {} };
        const result = hook(payload);
        assert.ok(result.config.tools.some((t) => t.urlContext));
    });
    it("injects codeExecution tool", () => {
        const hook = buildGeminiOnPayload({ ...BASE_CONFIG, codeExecution: true }, "google");
        assert.ok(hook);
        const payload = { config: {} };
        const result = hook(payload);
        assert.ok(result.config.tools.some((t) => t.codeExecution));
    });
    it("injects all three tools simultaneously", () => {
        const hook = buildGeminiOnPayload({ ...BASE_CONFIG, googleSearch: true, urlContext: true, codeExecution: true }, "google");
        assert.ok(hook);
        const payload = { config: {} };
        const result = hook(payload);
    ...
    it("works with google-vertex provider", () => {
        const hook = buildGeminiOnPayload({ ...BASE_CONFIG, googleSearch: true }, "google-vertex");
        assert.ok(hook);
    });
});
//# sourceMappingURL=gemini-onpayload.test.js.map